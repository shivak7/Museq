...
void to_json(json& j, const Note& note) {
    j = json{{"pitch", note.pitch}, {"duration", note.duration}, {"velocity", note.velocity}};
}
void from_json(const json& j, Note& note) {
...
void to_json(json& j, const Instrument& instrument) {
    j = json{{"name", instrument.name}, {"sequence", instrument.sequence}, {"type", (int)instrument.type}, {"portamento", instrument.portamento_time}};
