...
void OggWriter::write(AudioRenderer& renderer, const Song& song, const std::string& file_path, float sample_rate) {
    ...
    vorbis_info_init(&vi);
    // Use a higher quality (0.4) for better sound
    ...
    while (offset < pcm_buffer.size()) {
        int to_write = std::min((int)(pcm_buffer.size() - offset), chunk_size);
        float** buffer = vorbis_analysis_buffer(&vd, to_write);
        for (int i = 0; i < to_write; ++i) {
            buffer[0][i] = pcm_buffer[offset + i];
        }
        vorbis_analysis_wrote(&vd, to_write);
        offset += to_write;
    ...
    // Final flush to ensure every last byte is written
    while (ogg_stream_flush(&os, &og)) {
        fwrite(og.header, 1, og.header_len, ogg_file);
        fwrite(og.body, 1, og.body_len, ogg_file);
    }
