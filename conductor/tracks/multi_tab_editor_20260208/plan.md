...
- [ ] Task: Conductor - User Manual Verification 'Basic Tab Switching' (Protocol in workflow.md)

## Phase 2: Tab Lifecycle and Dirty State
- [ ] Task: Implement "Dirty" state tracking: detect text changes and mark the active tab with an asterisk (`*`).
- [ ] Task: Implement the close button (`x`) on tabs.
- [ ] Task: Create a "Save Changes?" modal popup that appears when closing a dirty tab, offering Save, Discard, and Cancel.
- [ ] Task: Add "New Tab" functionality (keyboard shortcut or sidebar action) creating an "Untitled" empty script.
