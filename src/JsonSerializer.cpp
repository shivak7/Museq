...
void to_json(json& j, const LFO& lfo);
void from_json(const json& j, LFO& lfo);
...
void to_json(json& j, const Filter& filter) {
    j = json{{"type", (int)filter.type}, {"cutoff", filter.cutoff}, {"resonance", filter.resonance}};
}
void from_json(const json& j, Filter& filter) {
    filter.type = (FilterType)j.at("type").get<int>();
    filter.cutoff = j.at("cutoff").get<float>();
    filter.resonance = j.at("resonance").get<float>();
}

void to_json(json& j, const LFO& lfo) {
    j = json{{"target", (int)lfo.target}, {"waveform", (int)lfo.waveform}, {"frequency", lfo.frequency}, {"amount", lfo.amount}};
}
void from_json(const json& j, LFO& lfo) {
    lfo.target = (LFOTarget)j.at("target").get<int>();
    lfo.waveform = (Waveform)j.at("waveform").get<int>();
    lfo.frequency = j.at("frequency").get<float>();
    lfo.amount = j.at("amount").get<float>();
}
...
void to_json(json& j, const Instrument& instrument) {
    ...
    if (instrument.type == InstrumentType::SYNTH) {
        j["synth"] = {
            {"waveform", (int)instrument.synth.waveform}, 
            {"envelope", instrument.synth.envelope},
            {"filter", instrument.synth.filter},
            {"lfo", instrument.synth.lfo}
        };
...
void from_json(const json& j, Instrument& instrument) {
    ...
    // Partial implementation, tree loading not supported
}
